...
return {
  ...
  config = function()
    ...
    -- Find cspell binary (prefer global, fallback to mason)
    local function get_cspell_cmd()
      local global_cspell = vim.fn.exepath("cspell")
      if global_cspell ~= "" then
        return global_cspell
      end
      local mason_cspell = vim.fn.stdpath("data") .. "/mason/bin/cspell"
      if vim.fn.executable(mason_cspell) == 1 then
        return mason_cspell
      end
      return "cspell" -- Let it fail with a clear error if not found
    end
